...
export default function CompleteStep({ garmentId, onClose }: { garmentId: string; onClose: () => void }) {
            ...
            <div className="h-[40vh] relative flex-shrink-0">
              ...
              <div className="absolute inset-0 pointer-events-none overflow-hidden">
                {Array.from({ length: 16 }).map((_, i) => (
                  <motion.div
                    key={i}
                    initial={{ x: `${10 + Math.random() * 80}vw`, y: -20, rotate: Math.random() * 360, opacity: 1 }}
                    ...
                    style={{ backgroundColor: ['#BF8B5E', '#4E7D4F', '#D4903A', '#C5574A', '#C4A882'][i % 5] }}
                  />
                ))}
              </div>
            </div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mb-8">
                ...
                <div className="flex gap-3 justify-center">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <motion.span
                        animate={{ scale: star <= (hoveredStar || rating) ? 1.15 : 1 }}
                ...
                {rating > 0 && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mt-3 text-label text-ink-2">
                    {['', "That's a start!", "Getting there!", 'Good work!', 'Brilliant!', 'Perfect result!'][rating]}
                  </motion.p>
                )}
              </motion.div>

              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="flex flex-col gap-3 pb-8">
                ...
                <PrimaryButton onClick={handleGoHome}>Start your next project</PrimaryButton>
                <button onClick={handleGoHome} className="w-full py-4 text-label font-medium text-ink-3">
                  Back to home
                </button>
              </motion.div>
            </div>
